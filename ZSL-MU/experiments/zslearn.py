...
def train(exp: ExperimentInfo, data_root: str, img_split: str, logger):
    implementation = import_module(f"src.implementations.zslearn.{exp.method.name}")
    state = implementation.prepare_train(exp, data_root, img_split)
    zslearn_module = implementation.train(state, logger)
    return zslearn_module
...
def evaluate(exp: ExperimentInfo, zslearn_module, data_root: str, img_split: str, logger, replace: bool = True):
    implementation = import_module(f"src.implementations.zslearn.{exp.method.name}")
    state = implementation.prepare_evaluate(exp, zslearn_module, data_root, img_split, replace)
    zslearn_metrics = implementation.evaluate(state, logger)
    return zslearn_metrics
...
def test(exp: ExperimentInfo, data_root: str, run: list[str], branch: str): pass

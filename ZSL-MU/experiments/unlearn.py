...
def train(exp: ExperimentInfo, data_root: str, img_split: str, logger):
    ...
    unlearn_module = implementation.train(state, logger)
    return unlearn_module
...
def evaluate(exp: ExperimentInfo, unlearn_module, data_root: str, img_split: str, logger, replace: bool = True):
    implementation = import_module(f"src.implementations.unlearn.{exp.method.name}")
    state = implementation.prepare_evaluate(exp, unlearn_module, data_root, img_split, replace)
    unlearn_metrics = implementation.evaluate(state, logger)
